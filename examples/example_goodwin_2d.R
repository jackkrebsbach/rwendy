...
u0_vec <- as.vector(U_star)
max_iter <- 100
tol <- 1e-8
lambda <- 1e-8

u <- u0_vec
pn <- rep(1,length(p_star))
...
for (iter in seq_len(max_iter)) {
  ...
  JtJ   <- t(J_w) %*% J_w
  delta <- solve(JtJ + lambda * diag(length(theta_w)), t(J_w) %*% G_w)
...
# res <- solveWendy(f, sol[,-1] + noise, tt, method = "JOINT")
